...
-- é assim que podemos acessar propriedades nesteadas em objects, com o snowflake.

SELECT RAW_FILE:job.salary as JOB from OUR_FIRST_DB.PUBLIC.JSON_RAW;
-- SINTAXE ALTERNATIVA (com ':' em vez de '.'):
SELECT RAW_FILE:job:salary as JOB from OUR_FIRST_DB.PUBLIC.JSON_RAW;
...
FROM OUR_FIRST_DB.PUBLIC.JSON_RAW; -- formato: 'exemplo2';
...
-- PARA NAO PERDER INFO, TEMOS O SEGUNDO APPROACH:
...
--  SAO ESSAS AS 2 MANEIRAS QUE USAMOS PARA TRABALHAR COM ARRAYS....
